...
def get_top_searched_keywords(request):
    ...
    aggs_size = int(request.GET.get('size', 10))
    result = es_aggregation_count_util.get_aggcount_for_date_range_for_customer(customer_id, from_date, to_date,
                                                                   range_field='user_visit_time_date',
                                                                   aggs_field='keyword',aggs_size=aggs_size)
    description = {"key": ("string", "Keyword"),
...
def get_top_customers_by_search_count(request):
    ...
    to_date = request.GET.get('to_date')
    aggs_size = int(request.GET.get('size', 10))
    result = es_aggregation_count_util.get_aggcount_for_date_range(from_date, to_date,
                                                                   range_field='user_visit_time_date',
                                                                   aggs_field='customer_id',aggs_size=aggs_size)
...
@view_config(route_name='search_response', renderer='searchit:templates/search_response.mako')
def get_search_response(request):
    result = es_aggregation_count_util.get_aggcount_for_customer_id()
    customers = []
    for doc in result:
        customers.append(doc['key'])
    return {'customers': customers}

@view_config(route_name='keyword_response', renderer='searchit:templates/keyword_response.mako')
def get_keyword_response(request):
    result = es_aggregation_count_util.get_aggcount_for_customer_id()
    customers = []
    for doc in result:
        customers.append(doc['key'])
    return {'customers': customers, "sizes": [5,10,15,20]}

@view_config(route_name='customers_response', renderer='searchit:templates/customers_response.mako')
def get_customers_response(request):
    return {"sizes": [5,10,15,20]}
